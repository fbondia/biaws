        ...
        <table className="optionListTable">
          ...
          <tbody>
            {draft.items.map((item, index) => (
              <tr
                className={[
                  draggedIndex === index ? "draggingOptionItem" : "",
                  dropTargetIndex === index ? "dropTargetOptionItem" : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
              ...
              >
                <td className="optionDragColumn">
                  <span
                    ...
                    aria-keyshortcuts={
                      canManage ? "ArrowUp ArrowDown" : undefined
                    }
                    className={
                      canManage
                        ? "optionDragHandle"
                        : "optionDragHandle disabled"
                    }
                    draggable={canManage}
                ...
                {hasColors ? (
                  <td className="optionColorFields">
                    {["foreground", "background", "border"].map((color) => (
                ...
                {hasEmlDetection ? (
                  <td className="optionEmlDetectionFields">
                    ...
                    />
                    <small>
                      Uma expressão por linha. Use <code>{"(?<code>...)"}</code>{" "}
                      para coletar o código.
                    </small>
                  </td>
                ) : null}
                <td>
          ...
          {testSubject ? (
            ...
            >
              {detectionTest?.error
                ? detectionTest.error
                : detectionTest
                  ? `Tipo: ${detectionTest.label}${detectionTest.code ? ` · Código: ${detectionTest.code}` : " · Código não coletado"}`
                  : `Nenhuma regra encontrada · será usado o tipo padrão “${
                      draft.items.find(
                        (item) => item.value === draft.defaultValue,
                      )?.label || draft.defaultValue
                    }” e um código sintético.`}
            </div>
          ) : null}
        </section>
      ) : null}
